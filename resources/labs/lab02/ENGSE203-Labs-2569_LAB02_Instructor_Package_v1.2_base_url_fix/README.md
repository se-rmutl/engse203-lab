...
## v1.2 repair — Invalid base URL in local Vite development

This version corrects `src/api.js` so data loading works at a local URL such as `http://localhost:5173/engse203-lab02-solution/`. The updated code passes the Vite public base path directly to `fetch()` instead of using it as the base argument of `new URL()`. See [`FIX_INVALID_BASE_URL.md`](./solution/engse203-lab02-solution/FIX_INVALID_BASE_URL.md).
