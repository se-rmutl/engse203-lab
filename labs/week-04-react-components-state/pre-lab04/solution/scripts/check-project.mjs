...
const sourceFiles = await Promise.all(
  requiredFiles
    ...
    .map(async (file) => readFile(join(root, file), 'utf8').catch(() => '')),
);
...
const checks = [
  ...
  ['stable key', /key=\{(?:task\.)?id\}|key=\{task\.id\}/],
...
if (failures.length > 0) {
  console.error('Pre-LAB 04 check: NOT READY');
  failures.forEach((failure) => console.error(`- ${failure}`));
  process.exit(1);
}

console.log('Pre-LAB 04 check: PASS — components, state, events and immutable updates found');
